...
const handleFormState = ({ submit, input }, formState, i18nInstance) => {
  ...
  switch (formState) {
    case 'filling':
      ...
      newSubmit.textContent = i18nInstance.t('form.submit');
      input.focus();
      break;
    case 'sending':
      newSubmit.disabled = true;
      newSubmit.textContent = i18nInstance.t('form.loading');
      break;
    default:
...
const handleErrors = ({ input, feedback }, error, i18nInstance) => {
  ...
  newFeedback.classList.remove('text-success');
  ...
  newFeedback.textContent = i18nInstance.t(`errors.${error}`);
  input.focus();
};
...
export default (elements, state, i18nInstance) => {
  const {
    input, submit, feedback, feeds, posts, modal,
  } = elements;
  return (path, value) => {
    switch (path) {
      case 'form.url':
